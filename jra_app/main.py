...
# 閾値の定数定義
THRESHOLD_WIN = -0.10  # 単勝・複勝の断層基準
THRESHOLD_UMATAN = -1.0  # 馬単の断層基準
...
if submit_button:
    ...
    if text_umatan:
        ...
        if df_uma is not None:
            ...
            if show_only_red_uma:
                # 馬単用の閾値を定数から取得
                mask = (df_uma['比差'] <= -0.1) | (df_uma['裏差'] <= THRESHOLD_UMATAN)
    ...
    st.info("👆 ボックスにデータを貼り付けて、「🚀 分析開始」ボタンを押してください")
